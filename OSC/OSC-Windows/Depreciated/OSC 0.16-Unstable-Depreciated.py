...
def get_vram_wmi():
    """Returns VRAM in MB using WMI (via pywin32 COM)"""
    try:
        wmi = win32com.client.Dispatch("WbemScripting.SWbemLocator")
        wbem = wmi.ConnectServer(".", "root\\CIMV2")
        gpus = wbem.ExecQuery("SELECT AdapterRAM, Name FROM Win32_VideoController")
        for gpu in gpus:
            if gpu.AdapterRAM:
                total_vram = int(gpu.AdapterRAM) // 1024**2
                return f"VRAM {total_vram} MB"
    except Exception:
        return "VRAM N/A"
    return "VRAM N/A"
